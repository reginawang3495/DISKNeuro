...
public class WingsAdapter {
  ...
  public Map<String, Variable> getWorkflowInputs(String username, String domain, String id) {
  ...
  private String getWorkflowRunWithSameBindings(
      String username, String domain, String wflowname, 
      List<NameValuePair> vbindings) {
    
    String templateid = WFLOWID(username, domain, wflowname);
    // Get all successful runs for the template (and their variable bindings)
    String query = 
    ...
    JsonArray qbindings = 
        result.get("results").getAsJsonObject().get("bindings").getAsJsonArray();

    for(JsonElement qbinding : qbindings) {
      JsonObject qb =  qbinding.getAsJsonObject();
      if(qb.get("run") == null)
        continue;
      String runid = qb.get("run").getAsJsonObject().get("value").getAsString();
      String bindstrs = qb.get("bindings").getAsJsonObject().get("value").getAsString();
      ...
      for(NameValuePair vbinding : vbindings) {
        ...
        if(value == null ||
            !value.equals(vbinding.getValue())) {
          match = false;
          break;
        }
      }
  ...
  public String runWorkflow(String username, String domain, String wflowname,
      List<VariableBinding> vbindings, Map<String, Variable> inputVariables) {
    try {
      String templateid = WFLOWID(username, domain, wflowname);
      List<NameValuePair> formdata = this.createFormData(username, domain, 
          templateid, vbindings, inputVariables);
      ...
      String runid = this.getWorkflowRunWithSameBindings(username, domain, 
          wflowname, formdata);
      if(runid != null) {
  ...
  public String fetchDataFromWings(String username, String domain, String dataid) {
...
public String addOrUpdateData(String username, String domain, 
  ...
  private List<NameValuePair> createFormData(String username, String domain, 
      String templateid, List<VariableBinding> bindings, Map<String, Variable> inputs) {
    List<NameValuePair> data = new ArrayList<NameValuePair>();
  ...
  private String upload(String username, String pageid, 
